...
from datasets.dataset_synapse import Synapse_dataset, RandomGenerator_test
...
def test_with_no_label(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        with torch.no_grad():
            ...
            image = image.float()
            outputs = model(image)
            outputs = torch.softmax(outputs, dim=1)  #单标签分类
            ...
            save_pic = Image.fromarray(np.uint8(outputs))

            save_pic.save(this_path + "/{}.png".format(1))
